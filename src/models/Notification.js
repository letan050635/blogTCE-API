...
class Notification {
   ...
   * @returns {Object} - Thông tin thông báo đã tạo
   */
  static async create(data) {
    const { title, brief, content, date, updateDate = null, isNew = true, useHtml = false } = data;
    ...
    const result = await query(
      `INSERT INTO notifications (title, brief, content, date, updateDate, isNew, useHtml) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [title, brief, content, formattedDate, formattedUpdateDate, isNew, useHtml]
    );
   ...
   * @returns {Object|null} - Thông tin thông báo hoặc null
   */
  static async findById(id, userId = null) {
    ...
    if (notification) {
      ...
      if (notification.updateDate) {
        notification.updateDate = moment(notification.updateDate).format('DD/MM/YYYY');
      }
    }
   ...
   * @returns {Object} - Thông tin thông báo sau khi cập nhật
   */
  static async update(id, data) {
    const allowedFields = ['title', 'brief', 'content', 'date', 'updateDate', 'isNew', 'useHtml'];
    const updateData = {};
    const params = [];
  ...
  static async delete(id) {
    ...
    await query('DELETE FROM user_read_status WHERE itemId = ? AND itemType = ?', [id, 'notification']);
  ...
  static async findAll(options = {}) {
    ...
    if (search) {
      if (searchInContent) {
        conditions.push(`(n.title LIKE ? OR n.brief LIKE ? OR n.content LIKE ?)`);
  ...
  static async markAllAsRead(userId) {
    ...
    const values = unreadNotifications.map(notification => {
